...
- [x] Все страницы работают (/, /about, /contact, /cart, /favorites, /product/[id])
- [x] SSG генерация страниц товаров из Markdown файлов
- [x] Корзина работает с Zustand (добавление, удаление, количество, доставка)
- [x] Избранное работает с Zustand (toggle, синхронизация между страницами)
- [x] Персистентность в localStorage (корзина, избранное, cookie)
- [x] Моковые API работают (/api/feedback, /api/order)
- [x] Адаптивность (mobile-first, 1-4 колонки)
- [x] Темизация работает (light/dark, teal акцент)
...
- [x] 7.1 Создать app/product/[id]/page.tsx
...
- [x] 7.2 Создать Client Component для интерактива
  ...
  - Toast уведомления
- [x] 7.3 Обработать 404
  - Если товар не найден → notFound() из next/navigation
- [x] 7.4 Исправить замечания после код-ревью (stage-7-review.md)
  - Исправлена гидратационная ошибка (mounted state)
  - Изображение отображается полностью (objectFit: contain)
  - Изображение уменьшено на 10% при > 1000px
  - Адаптивный блок количества и кнопки
  - Круглые кнопки +/- с символами
  - Линтер и тесты проходят успешно
...
- ✅ Гидратационная ошибка исправлена
- ✅ Изображение полностью видно (objectFit: contain)
- ✅ Адаптивность работает (мобильные/десктоп)
- ✅ Кнопки +/- круглые и кликабельные
- ✅ Линтер проходит (0 ошибок, 1 warning)
...
- [x] 8.1 Создать app/favorites/page.tsx (Client Component)
...
## 🌓 Этап 14: Переключатель тем в UI

**Цель:** Добавить интерактивный переключатель светлой/тёмной темы
...
## 🎮 Этап 15: Hero-блок с SVG-анимацией

**Цель:** Создать одностраничный Hero-блок с векторной иллюстрацией микрокомпьютера

### Задачи

- [x] 15.1 Создать components/hero/HeroScene.tsx (Client Component)
  - SVG-иллюстрация микрокомпьютера (вид сверху):
    - PCB board с декоративными линиями дорожек
    - SoC chip, RAM chip, WiFi chip
    - USB×2, Ethernet, HDMI, USB-C порты
    - GPIO pins (2 ряда по 20)
    - Конденсаторы, пульсирующий LED индикатор
  - Плавная анимация появления SVG (fade + scale + translateY)
  - Параллакс при движении мыши (смещение SVG по курсору)
  - Декоративные плавающие частицы на фоне (30 штук, teal.500)
  - Glassmorphism оверлей внизу экрана:
    - backdrop-filter: blur(20px), полупрозрачный фон
    - Надпись «Микрокомпьютеры будущего» с иконкой Cpu
    - Описание + CTA кнопка «Перейти в каталог»
- [x] 15.2 Адаптивный фон — зависит от цветовой темы
  - `bg="bg.default"` — автоматически светлый/тёмный через Chakra semantic tokens
  - `transition="background-color 0.4s ease"` для плавного переключения
- [x] 15.3 Интегрировать HeroScene в app/page.tsx
  - HeroScene на весь экран (100vh)
  - Каталог ниже с якорем #catalog
- [x] 15.4 Написать тесты (8 тестов)
  - Заголовок, описание, CTA, ссылка, SVG, адаптивный фон, частицы, mount
- [x] 15.5 Удалить зависимость three (больше не используется)

**Проверка:**
- ✅ TypeScript компиляция без ошибок
- ✅ Линтер без ошибок (0 errors, 0 warnings)
- ✅ Все тесты проходят (89 тестов)
- ✅ Зависимость three удалена

---

## ⬆️ Этап 16: Кнопка подъёма наверх (ScrollToTop)

**Цель:** Добавить кнопку плавного скролла наверх при прокрутке страницы

### Задачи

- [x] 16.1 Создать components/ui/ScrollToTop.tsx (Client Component)
  - Круглая кнопка с иконкой стрелки вверх (ChevronUp из lucide-react)
  - Появляется при scrollY > 100px
  - Исчезает при scrollY <= 100 (включая top === 0)
  - Плавный скролл наверх при клике (window.scrollTo behavior: "smooth")
  - Фиксированная позиция в правом нижнем углу
  - Акцентный цвет teal
- [x] 16.2 Интегрировать ScrollToTop в ClientShell
  - Добавлен в ClientShell.tsx после ToastRenderer
- [x] 16.3 Написать тесты
  - 5 тестов: скрытие при 0, появление при >100, скролл вниз/наверх, клик

**Проверка:**
- ✅ TypeScript компиляция без ошибок
- ✅ Линтер без ошибок
- ✅ Все тесты проходят (81 тест)
...
| 15. Hero-блок SVG | ✅ Завершён | SVG микрокомпьютер, параллакс, адаптивный фон, 89 тестов |
| 16. ScrollToTop | ✅ Завершён | Круглая кнопка со стрелкой, порог 100px |
| 17. Полировка | ⬜ Не начат | |
